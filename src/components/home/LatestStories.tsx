...
export function LatestStories({ stories }: { stories: HomeStory[] }) {
  return (
    <section className="py-14 md:py-24" id="stories">
      <Wrap>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {stories.map((story) => (
            <article key={story._id} className="group flex flex-col">
              ...
              {story.excerpt ? (
                <p className="mt-2 text-[0.95rem] text-muted line-clamp-3">{story.excerpt}</p>
              ) : null}
            </article>
          ))}
        </div>
